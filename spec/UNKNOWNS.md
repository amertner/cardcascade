...
| the `2.600` width offset | `cad/parts/box.py`, `spec/BOX.md` "Still open" | `1.600` on `-X`, `1.000` on `+X` | all nine Box references | Asymmetric and unexplained; the closing bump is `1.000` proud, which accounts for one side. The sketch's own dimension names in the feature tree would name the other. |
