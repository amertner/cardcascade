...
| `Text right-aligned sample 2.step` | the same plate with a fourth line, `Orbitron Bold`, right-aligned to the same edge (2026-09-04). Its ink ends `0.0761 ±0.0004` em short of the edge — the tray's `TRAIL` mechanism, on the parts' own weight — and with the Open Sans line's `0.0646` it is what settled both constants as a quarter of the font's space advance |
