...
The H2C is a DUAL-NOZZLE printer and its nozzles do not reach the same bed:
`extruder_printable_area` is `x 0..325` for one and `x 25..330` for the
other, so only **x 25..325 is reachable by BOTH**. Objects are unaffected —
each is printed by one extruder — but every filament in use purges into the
PRIME TOWER, so on a two-filament plate the tower must sit in the
intersection. Only lid plates carry two filaments, so this is a lid-plate
defect exclusively.

Nothing upstream catches it: Studio's 3D view shows the plate as fine and it
surfaces only after slicing, as `return_code -102`, "Found G-code in
unprintable area of multi-extruder printers after slicing" — and MakerWorld
slices on upload, so the first sign is a rejected upload. Both placers bound
the tower by `make_cascade.tower_bounds()`; PIPELINE.md has the rest.
...
--fix writes through filaments.write_settings, so every other zip member is
copied byte-for-byte and nothing goes near Bambu Studio. Verify a repair by
slicing, as MakerWorld does — `BambuStudio --slice <plate> --outputdir <dir>
<project.3mf>`, then `return_code` in result.json must be 0, not -102. The
declared area is ~2 mm stricter than the slicer, which keeps that as margin.
"""
import argparse
import json
import math
...
def sat_overlap(a, b, gap=0.0):
    """a, b: (cx, cy, hx, hy, theta) oriented boxes — make_cascade's test."""
    for o in (a, b):
        c, s = math.cos(o[4]), math.sin(o[4])
        for ax in ((c, s), (-s, c)):
            a0, a1 = _proj(a, ax)
...
def _local_bbox(root_xml, meshes, oid):
    """An object's own bbox, before its build item: mesh or <component>."""
    ...
    for cm in re.finditer(r'<component p:path="([^"]+)" objectid="(\d+)"'
        ...
        # transforms here are translations, so the bbox stays axis-aligned
...
def plates(zf):
    """[(plate id, [(object id, obb) ...], {extruders})], plate-local."""
...
def problems(path):
    """[(plate id, x, y, reason)] per multi-filament plate whose tower escapes
    the intersection."""
...
def place(ps, obbs):
    """A legal tower position for one plate, or None: 4 mm grid, 15 mm
    clearance, furthest from the bed centre."""
    bx0, by0, bx1, by1 = tower_bounds(ps)
    w = tower_size(ps)
    area = [tuple(map(float, p.split("x"))) for p in ps["printable_area"]]
    bed_w = max(p[0] for p in area)
