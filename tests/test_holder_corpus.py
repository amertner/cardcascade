...
buckets = Counter()
absent = []
for game, legacy, built, p, first in catalogue():
    cpath = INDIV / FOLDER[game] / legacy
    bpath = BUILD / game / built
    if not cpath.exists():
        absent.append(f"{game}/{legacy}")
        continue
    if not bpath.exists():
        fails.append(f"{game}/{built} was not built")
        print(f"    FAIL {game}/{built} was not built — run "
              "`python -m cad.build --part holder` first")
        continue
    d = D.derive(p)
    _cn, cv, ct = mesh(cpath)
    _bn, bv, bt = mesh(bpath)
    cbox, bbox = box_of(cv), box_of(bv)
    end = round((cbox[1] - cbox[0]) - d.calSlotwidth * p.HorizontalSlots, 3)
    buckets[end] += 1

    # The engraved version is the studio's, not the product's: every cached
    # holder reads `CC 6.6` and every hand-exported STEP reads `CC 7.0`. Compare
    # like for like, and tessellate the same way the writer does.
    at66 = holder.build(replace(p, Version="6.6"), first)
    mv, mt = mesh3mf.triangulate(at66)
    want_vol = mesh_volume(mv, mt)
    got_vol = mesh_volume(cv, ct)
    delta = (want_vol / got_vol - 1) * 100

    # Where Onshape's own size rule makes the two text blocks collide, ours is
    # smaller — so LESS is engraved away and the part is heavier by that ink.
    by_depth = (holder.holder_depth(p, d, first) - 2.0) / TX.CAP
    capped = holder.text_size(p, d, first) < by_depth - 1e-6

    tag = f"{game}/{legacy}"
    print(f"  {legacy:34s} {end:+7.3f} {got_vol:11.3f} {want_vol:11.3f} "
          f"{delta:+8.4f}%  {'text' if capped else '    '}  {built}")
    ...
    if end == CURRENT_END:
        ...
        # The pre-2026-08-24 studio, and the difference is the end block ALONE:
        # 0.100 in at each end, nothing else anywhere, and exactly the volume
        # that removes. Asserting the second half is the point — a stale file
        # that also differed in the lattice or the lip would pass a width check.
