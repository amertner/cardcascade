...
`build/cascades/` (`cad.cascade`) — and since 2026-09-05 the name carries the
version, because a downloaded print profile is a file and the file is all its
owner has to say which generation it is.

That makes the name a claim about the parts inside, and this suite is what
holds it:

  * every shipped project is EXACTLY what the rule generates from its row, so
    the tree cannot drift from the generator (48 files, no tolerance);
  * the version each name carries is the row's own GENERATION, not a constant:
    a row pinned `6.6` in parts.csv has to be named `v6.6` (`290 Card (Mat)` is
    the pinned one, and has no project yet, so the rule is driven directly);
  ...
  * `cad.cascade`'s titles, which are the same rule at `p.Version` — including
    that `--version 7.1` really does name a different file from the 7.0 one,
    which is the whole point of the 7.1 stamp.
...
check("no version given, no version written (the previous scheme, for find_legacy)",
...
for game, spec in C.GAMES.items():
    ...
    for c in plan.cascades:
        name = C.cascade_filename(game, c["ctx"]["short_name"], c["sleeved"],
                                  c["model"], c["ctx"]["generation"], c["ctx"]["label"])
        expected.setdefault(spec["folder"], {})[name] = c
...
print("\n=== the version each name carries is the row's generation ===")
...
for build, want in (("", OC.CURRENT), ("6.6", "6.6"), ("Un:6.6 Sl:7.0", "6.6")):
    ...
    check(f"Build {build!r} names the unsleeved cascade v{want}",
...
check("a 7.1 set is named apart from the 7.0 one",
