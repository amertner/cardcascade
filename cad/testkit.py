...
from the same part modules as the catalogue at `CURRENT`, so what it proves is
what the catalogue would print — cut down to what the lips need. It ships the
**Box** without label holders or lattice, SLICED from `FLOOR_Z` up; the
**Holder** and **RearHolder**, sliced so their cut face lands where their base
was; the **Pusher**; and a **stub** of the lid carrying the pusher sockets.
Everything takes a catalogue role (`layout.ROLES`) and packs onto ONE plate.
`spec/BOX.md`, "The ribs move forward", says what to look for.
"""
import argparse
import math
import sys
...
# Where the sliced box's floor goes (its underside), in the box's own Z: above
# the lattice's top row, and far enough below the front holder's wall top that
# the wall keeps 10+ mm below its rest.
FLOOR_Z = 60.0

# (label, why, Primary). LabelHolders 0: the label holders test nothing here.
KITS = {
    # The flattest one-slot cascade whose box builds: its final edge round
    # fails once the pusher is longer than about 20. The lip's reach in Y and
    # its seat are the same at any slope.
...
def sliced_box(d):
    """The box from FLOOR_Z up, on its own real floor lifted to FLOOR_Z."""
...
def holder_cut_z(d):
    """Where a holder is cut, in its own frame: the sliced floor's top is where
    its base was, so `FLOOR_Z + floor_top` in the box is that Z."""
...
def stub(d):
    """The lid's floor and sockets, rim cut off STUB_H up — what locates a box
    and stands a pusher."""
...
def one_plate(objs, name):
    ...
    printed in one go, not by role, and `layout.plate` packs it."""
