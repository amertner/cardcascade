...
**Catalogue** (`CATALOGUE.md`, the public "does one fit my game" table, linked
from README, r/cardcascade and the MakerWorld listings): GENERATED by
`make_catalogue.py` from parts.csv through `cad.derive`, the sizes by the
posters' formulas; `catalogue.json` holds the words and the links
(`makerworld`, `made_for`). Rerun it after any parts.csv change:
`tests/test_catalogue.py` fails while it is stale. `outreach/` holds the
Reddit drafts and plan (2026-09-26).
